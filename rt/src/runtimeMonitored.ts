...
function initRuntime() {
  ...
  rt_localStorageDelete = mkBase((env, arg) => {
    assertNormalState("localStorageDelete");
    assertIsString(arg);
    localStorage.removeItem(arg.val);
    rt_ret(__unit);
  }, "rt_localStorageDelete");

  rt_webBuilder = mkBase((env, arg) => {
    assertNormalState("webBuilder");
    console.log("webBuilder");
    let htmlContent = arg.val;
    let newWindow = window.open(); // open new tab
    newWindow.document.title = "Troupe Web Builder";
    newWindow.stop(); // stops loading
    if (!Array.isArray(htmlContent)) {
      newWindow.document.write(htmlContent.outerHTML); // write html in body
    rt_ret(__unit);
    } else {
      let completeContent = "";
      htmlContent.forEach(element => {
        completeContent += element.val.outerHTML;
      });
      newWindow.document.write(completeContent); // write html in body
    }
    rt_ret(__unit);
  ...
  rt_getElementsByTagName = mkBase((env, arg) => {
